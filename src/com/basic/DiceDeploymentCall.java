...
public class DiceDeploymentCall {
	...
	public void dicedeploymentservice(String token) {
		...
		try {
			...
			if (conn.getResponseCode() != 200) {
				Globals.deploymentnodelist = Globals.deploymentnodelist
						.concat("Failed : HTTP error code : " + conn.getResponseCode());
			...
			for (int i = 0; i < vm.length; i++) {
				if (vm[i].getBlueprint() != null) {
					...
					if (vm[i].getBlueprint().getIn_error() != null
							| (vm[i].getBlueprint().getIn_error().toString().equals(errorstate))) {
	...
	public void dicedeploymentfaultcaller(String token, String id, String name, String fault, String username,
			String password) {
		try {
			...
			for (int i = 0; i < node.length; i++) {
				...
				// Globals.deploymentnodelist=
				// Globals.deploymentnodelist.concat(node[i].toString());
				// Globals.deploymentnodelist=
				// Globals.deploymentnodelist.concat(fault);
			}
			...
			// Globals.deploymentnodelist=
			// Globals.deploymentnodelist.concat(e.toString());
	...
	public void dicedeploymentfaultcaller1(String token, String id, String name, String fault, String username,
			String sshkey) {
		try {
			...
			for (int i = 0; i < node.length; i++) {
				...
				if (ArrayUtils.contains(node[i].getComponents(), name)) {
					...
					// ("/Users/darrenw/Downloads/PrimeTest/Uploads/test");
					String host;
					switch (fault) {
	...
	public void dicedeploymentlist(String token, String id) {
		try {
			...
			for (int i = 0; i < node.length; i++) {
				...
				// {
				// TODO Read properties to compare and use this to start faults
				// on VM
				...
				// {
				// Globals.deploymentnodelist=
				// Globals.deploymentnodelist.concat("Not found details from
				// file");
				// }
				// }
				// } finally
